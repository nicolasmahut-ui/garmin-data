# Coach — J-35 avant Gérardmer XL

*samedi 1 août 2026* · course le samedi 5 septembre 2026

---

## 1. Ta séance

### 2026-08-01 · lap_swimming · 47 min

*Séance hors plan.*

**Le détail**

| | Séance | Réf. 10 dernières |
|---|---|---|
| Distance | **2.3 km** | 1.7 km |
| FC moyenne | **138** | 125 |
| FC max | **158** | 149 |
| TSS | **64** | 38 |
| Dérive cardiaque | **16.3 bpm** | 14.5 bpm |
| Allure | **2:03/100 m** | 2:19/100 m |

Zones FC : **Z1** 7 % · **Z2** 22 % · **Z3** 69 % · **Z4** 2 %

Tours, signal fin et trace : `seances/2026-08-01_lap_swimming_23808870314.md`

---

## 2. Ce que ça projette sur Gérardmer

| Segment | Projeté | Cible | Écart | Base |
|---|---|---|---|---|
| Natation 1,9 km | **0 h 42** | 0 h 41 | +1 min | 2:11/100 m |
| T1 | **0 h 05** | 0 h 05 | +0 min |  |
| Vélo 90 km / 1 200 m | **3 h 35** | 4 h 10 | -35 min | modèle sur 15 sorties |
| T2 | **0 h 04** | 0 h 04 | +0 min |  |
| Course 21,1 km | **2 h 24** | 2 h 25 | -1 min | 6:49/km projeté |
| **TOTAL** | **6 h 50** | 7 h 25 | **-35 min** | |

**Tu es en avance sur la cible.** Elle sera révisée à la hausse.

*Modèle ajusté sur tes 15 dernières sorties extérieures : 2.04 min par km, plus 2.64 min par 100 m de dénivelé. Recalculé à chaque sortie.*

---

## 3. Ton sommeil et cette séance

**La nuit qui a précédé** (2026-08-01)

| | Valeur | Moyenne 28 j | Cible |
|---|---|---|---|
| Durée | 8.9 h | 7.0 h | 8 h |
| Profond | 141 min | 92 min | 16-33 % |
| REM | 99 min (18 %) | 87 min | 21-31 % |
| Score | 73 | 74.7 | |
| VFC | 54 ms | 90 ms | base 85-132 |
| FC repos | - | 44 | |
| Readiness | 25 | 56 | |
| Récupération restante | 22 h | | |

**Ce que ton historique démontre**

Sur 72 séances appariées à la nuit précédente, en séparant au readiness médian de 56 :

| | Après bonne nuit | Après mauvaise nuit | Écart |
|---|---|---|---|
| Découplage moyen | **7.8 %** | **6.7 %** | -1.1 pt |
| Sommeil | 7.7 h | 6.8 h | |
| REM | 94.1 min | 58.9 min | |
| VFC | 97.6 ms | 76.2 ms | |

**Conclusion : pas d'effet net mesurable pour l'instant.** Tu encaisses bien les nuits moyennes.

---

## 4. Où tu en es

Bloc en cours depuis le 2026-06-22 (lendemain de Deauville).

| Jalon | État | Sur ce bloc | Record absolu |
|---|---|---|---|
| Sortie vélo avec 1 200 m de D+ en extérieur | ⬜ | 581 m | 842 m |
| Sortie vélo de 3 h 30 ou plus | ✅ | 3.8 h | 3.8 h |
| Course de 1 h 45 ou plus | ⬜ | 1.4 h | 1.4 h |
| Brique avec 45 min de course enchaînée | ⬜ | — | 42 min |
| Natation continue de 1 900 m | ✅ | 2300 m | 2300 m |
| Trois semaines à 3 natations ou plus | ⬜ | 1 sem. sur 6 | 1 sem. au total |

**Charge** : CTL **29.1** (+0.9 sur 7 j) · ATL 34.4 · TSB -5.4

Trajectoire : 29 aujourd'hui, cible **32** au 30 août, puis affûtage. Tu es dans les temps.

---

## 5. Demain

**2026-08-02** — Reprise dehors, endurance

- cycling · 150 min · 450 m D+ · FC 125-140

⚠️ **Ajustement** : VFC sous ta base, readiness bas. Réduis l'intensité d'un cran et garde la durée, ou décale la séance dure de 24 h.

---

Version 2026.08.01-r13 · régénéré à chaque synchronisation. Détail : `seances/`, `nuits/`, `bilan.md`.
