# Coach — J--17 avant Gérardmer XL

*mardi 22 septembre 2026* · course le samedi 5 septembre 2026

---

## 1. Ta séance

### 2026-09-20 · road_biking · 182 min

*Séance hors plan.*

**Le détail**

| | Séance | Réf. 10 dernières |
|---|---|---|
| Distance | **79.3 km** | 63.3 km |
| Dénivelé | **640 m** | 505 m |
| FC moyenne | **117** | 120 |
| FC max | **166** | 150 |
| Puissance norm. | **228 W** | 162 W |
| Cadence | **81** | 60 |
| TSS | **394** | 231 |
| Découplage | **2.4 %** | 4.6 % |
| Dérive cardiaque | **10.2 bpm** | 1.7 bpm |
| Meilleur 20 min | **235 W** | 239 W |
| Allure | **2:17/km** | 2:09/km |
| Negative split | **oui** (6.5 %) | 4/7 fois |
| Météo | 16 °C · 88 % hum. | |

Zones FC : **Z1** 42 % · **Z2** 39 % · **Z3** 16 % · **Z4** 2 %

**Lecture** — Découplage 2.4 % — allure parfaitement tenue, c'est exactement le comportement de course. Negative split réussi : c'est ce qui a manqué à Fréjus.

Tours, signal fin et trace : `seances/2026-09-20_road_biking_24429415168.md`

---

## 2. Ce que ça projette sur Gérardmer

| Segment | Projeté | Cible | Écart | Base |
|---|---|---|---|---|
| Natation 1,9 km | **0 h 43** | 0 h 41 | +2 min | 2:17/100 m |
| T1 | **0 h 05** | 0 h 05 | +0 min |  |
| Vélo 90 km / 1 200 m | **3 h 40** | 4 h 10 | -30 min | modèle sur 15 sorties |
| T2 | **0 h 04** | 0 h 04 | +0 min |  |
| Course 21,1 km | **2 h 22** | 2 h 25 | -3 min | 6:45/km projeté |
| **TOTAL** | **6 h 55** | 7 h 25 | **-30 min** | |

**Tu es en avance sur la cible.** Elle sera révisée à la hausse.

*Modèle ajusté sur tes 15 dernières sorties extérieures : 1.93 min par km, plus 3.89 min par 100 m de dénivelé. Recalculé à chaque sortie.*

---

## 3. Ton sommeil et cette séance

**La nuit qui a précédé** (2026-09-20)

| | Valeur | Moyenne 28 j | Cible |
|---|---|---|---|
| Durée | 7.0 h | 6.8 h | 8 h |
| Profond | 68 min | 79 min | 16-33 % |
| REM | 92 min (22 %) | 74 min | 21-31 % |
| Score | 88 | 75.2 | |
| VFC | 117 ms | 98 ms | base 82-131 |
| FC repos | - | - | |
| Readiness | 52 | 51 | |
| Récupération restante | 36 h | | |

**Ce que ton historique démontre**

Sur 75 séances appariées à la nuit précédente, en séparant au readiness médian de 56 :

| | Après bonne nuit | Après mauvaise nuit | Écart |
|---|---|---|---|
| Découplage moyen | **4.5 %** | **8.5 %** | +3.9 pt |
| Sommeil | 7.6 h | 6.7 h | |
| REM | 90.1 min | 74.0 min | |
| VFC | 102.3 ms | 89.6 ms | |

**Conclusion : une mauvaise nuit te coûte 3.9 points de découplage.** Sur les 4 h de vélo de Gérardmer, cet écart se paie directement sur les 21 km. Ton sommeil n'est pas un sujet de confort, c'est un paramètre de performance.

---

## 4. Où tu en es

Bloc en cours depuis le 2026-06-22 (lendemain de Deauville).

| Jalon | État | Sur ce bloc | Record absolu |
|---|---|---|---|
| Sortie vélo avec 1 200 m de D+ en extérieur | ✅ | 1280 m | 1280 m |
| Sortie vélo de 3 h 30 ou plus | ✅ | 3.9 h | 3.9 h |
| Course de 1 h 45 ou plus | ⬜ | 1.4 h | 1.4 h |
| Brique avec 45 min de course enchaînée | ✅ | 46 min | 46 min |
| Natation continue de 1 900 m | ✅ | 3000 m | 3000 m |
| Trois semaines à 3 natations ou plus | ⬜ | 0 sem. sur 6 | 1 sem. au total |

**Charge** : CTL **61.9** (+0.7 sur 7 j) · ATL 75.2 · TSB -13.3

Trajectoire : 62 aujourd'hui, cible **32** au 30 août, puis affûtage. Tu es dans les temps.

---

## 5. Demain

**2026-09-23** — repos ou séance libre très facile.

**Vigilance** : récupération incomplète. Fais la séance mais dans le bas des zones.

---

Version 2026.08.01-r17 · régénéré à chaque synchronisation. Détail : `seances/`, `nuits/`, `bilan.md`.
