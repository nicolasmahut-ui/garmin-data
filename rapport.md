# Coach — J--7 avant Gérardmer XL

*samedi 12 septembre 2026* · course le samedi 5 septembre 2026

---

## 1. Ta séance

### 2026-09-11 · tennis_v2 · 71 min

*Séance hors plan.*

**Le détail**

| | Séance | Réf. 10 dernières |
|---|---|---|
| Distance | **0.3 km** | 0.8 km |
| FC moyenne | **129** | 121 |
| FC max | **170** | 162 |
| Cadence | **14** | 21 |
| TSS | **50** | 42 |
| Découplage | **-6.5 %** | 9.6 % |
| Dérive cardiaque | **1.4 bpm** | -0.8 bpm |
| Negative split | **oui** (7.7 %) | 4/10 fois |

Zones FC : **Z1** 20 % · **Z2** 37 % · **Z3** 37 % · **Z4** 7 %

**Lecture** — Découplage -6.5 % — allure parfaitement tenue, c'est exactement le comportement de course. Negative split réussi : c'est ce qui a manqué à Fréjus.

Tours, signal fin et trace : `seances/2026-09-11_tennis_v2_24324984044.md`

---

## 2. Ce que ça projette sur Gérardmer

| Segment | Projeté | Cible | Écart | Base |
|---|---|---|---|---|
| Natation 1,9 km | **0 h 43** | 0 h 41 | +2 min | 2:15/100 m |
| T1 | **0 h 05** | 0 h 05 | +0 min |  |
| Vélo 90 km / 1 200 m | **3 h 39** | 4 h 10 | -31 min | modèle sur 15 sorties |
| T2 | **0 h 04** | 0 h 04 | +0 min |  |
| Course 21,1 km | **2 h 23** | 2 h 25 | -2 min | 6:46/km projeté |
| **TOTAL** | **6 h 53** | 7 h 25 | **-32 min** | |

**Tu es en avance sur la cible.** Elle sera révisée à la hausse.

*Modèle ajusté sur tes 15 dernières sorties extérieures : 1.95 min par km, plus 3.57 min par 100 m de dénivelé. Recalculé à chaque sortie.*

---

## 3. Ton sommeil et cette séance

**La nuit qui a précédé** (2026-09-11)

| | Valeur | Moyenne 28 j | Cible |
|---|---|---|---|
| Durée | 8.8 h | 6.8 h | 8 h |
| Profond | 108 min | 75 min | 16-33 % |
| REM | 109 min (21 %) | 76 min | 21-31 % |
| Score | 82 | 74.1 | |
| VFC | 130 ms | 98 ms | base 83-128 |
| FC repos | - | - | |
| Readiness | 43 | 50 | |
| Récupération restante | 39 h | | |

**Ce que ton historique démontre**

Sur 73 séances appariées à la nuit précédente, en séparant au readiness médian de 57 :

| | Après bonne nuit | Après mauvaise nuit | Écart |
|---|---|---|---|
| Découplage moyen | **5.1 %** | **7.6 %** | +2.5 pt |
| Sommeil | 7.6 h | 6.7 h | |
| REM | 88.5 min | 76.3 min | |
| VFC | 100.8 ms | 88.3 ms | |

**Conclusion : une mauvaise nuit te coûte 2.5 points de découplage.** Sur les 4 h de vélo de Gérardmer, cet écart se paie directement sur les 21 km. Ton sommeil n'est pas un sujet de confort, c'est un paramètre de performance.

---

## 4. Où tu en es

Bloc en cours depuis le 2026-06-22 (lendemain de Deauville).

| Jalon | État | Sur ce bloc | Record absolu |
|---|---|---|---|
| Sortie vélo avec 1 200 m de D+ en extérieur | ✅ | 1280 m | 1280 m |
| Sortie vélo de 3 h 30 ou plus | ✅ | 3.9 h | 3.9 h |
| Course de 1 h 45 ou plus | ⬜ | 1.4 h | 1.4 h |
| Brique avec 45 min de course enchaînée | ✅ | 46 min | 46 min |
| Natation continue de 1 900 m | ✅ | 3000 m | 3000 m |
| Trois semaines à 3 natations ou plus | ⬜ | 0 sem. sur 6 | 1 sem. au total |

**Charge** : CTL **56.5** (-3.1 sur 7 j) · ATL 75.3 · TSB -18.7

Trajectoire : 56 aujourd'hui, cible **32** au 30 août, puis affûtage. Tu es dans les temps.

---

## 5. Demain

**2026-09-13** — repos ou séance libre très facile.

**Vigilance** : récupération incomplète. Fais la séance mais dans le bas des zones.

---

Version 2026.08.01-r17 · régénéré à chaque synchronisation. Détail : `seances/`, `nuits/`, `bilan.md`.
